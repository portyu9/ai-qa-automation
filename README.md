...
Trusted default-branch admission in `trusted-pr-auto.yml` has two automatic classes. Owner-routine PRs require zero protected-root drift. Three finite governed-bot lanes—canonical Dependabot GitHub Actions updates, deterministic dependency promotions, and CodeQL auto-heal repairs—are discovered by a serialized trusted-main schedule and may cross their explicitly reviewed roots only after exact bot/branch/source provenance, full prospective-merge validation, exact governed-head CodeQL bound to a head/merge tree-equality proof, and terminal reproof. The dedicated GitHub App then publishes an exact PR/base/head/merge-bound **Trusted PR Gate** status. Unrecognized protected maintenance remains deny-by-default behind the independently deployed one-shot gate.
