...
Trusted default-branch admission in `trusted-pr-auto.yml` has two automatic classes. Owner-routine PRs require zero protected-root drift. Three finite governed-bot lanes—canonical Dependabot GitHub Actions updates, deterministic dependency promotions, and CodeQL auto-heal repairs—may cross their explicitly reviewed roots only after exact bot/branch/source provenance, trusted-main CI + CodeQL qualification, prospective-merge validation, and terminal reproof. The dedicated GitHub App then publishes an exact PR/base/head/merge-bound **Trusted PR Gate** status. Unrecognized protected maintenance remains deny-by-default behind the independently deployed one-shot gate.
