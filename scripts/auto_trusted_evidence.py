...
def _require_automatic_context(expected_base_sha: str) -> None:
    ...
    if os.environ.get("GITHUB_ACTOR") != EXPECTED_OWNER:
        raise PermissionError(
            "automatic protected evidence admission requires the repository owner actor"
        )
    if os.environ.get("GITHUB_SHA") != expected_base_sha:
        raise PermissionError(
            "automatic protected evidence admission requires the exact trusted main revision"
        )
...
def verify_automatic_trusted_evidence(
    *,
    ...
    if artifact_summary["build_manifest_source_tree"] != merge_tree_sha:
        raise ValueError(
            "persisted build manifest tree does not match the live prospective merge tree"
        )
...
def main() -> None:
    ...
    result = verify_automatic_trusted_evidence(
        ...
        evidence_run_id=_require_positive_int(
            args.evidence_run_id, label="evidence workflow run ID"
        ),
    )
