...
class AppTokenProvider:
    def __init__(
        ...
        *,
        ...
        if "BEGIN" not in private_key_pem or "PRIVATE KEY" not in private_key_pem:
            raise ValueError("GitHub App private key is malformed")
    ...
    def _mint_app_jwt(self, *, now: int) -> str:
        ...
        try:
            fd, key_path = tempfile.mkstemp(prefix="trusted-gate-key-", suffix=".pem")
            os.fchmod(fd, 0o600)
            with os.fdopen(fd, "wb", closefd=True) as handle:
                handle.write(self._private_key_pem.encode("utf-8"))
                handle.flush()
