...
The Lambda adapter deliberately separates cheap rejection and public configuration from the two `SecureString` values that require plaintext access:
...
4. An active one-shot policy uses a plain `String` parameter read with `WithDecryption=False`; its bytes must match the deployment SHA-256 pin and parse as the exact reviewed policy schema.
...
6. App/repository identity values use plain `String` parameters and are read without decryption. The GitHub App signing credential remains a `SecureString` and is decrypted only after HMAC, active-policy, wake-up, and installation admission. That signing credential is intentionally **not** cached across warm invocations.
