...
def test_ci_contract_rejects_removed_immediate_preinstall_revalidation(tmp_path: Path) -> None:
    root = _copy_workflows(tmp_path)
    path = _ci_path(root)
    text = path.read_text(encoding="utf-8").replace(
...
def test_ci_contract_rejects_build_authority_after_project_install(tmp_path: Path) -> None:
    root = _copy_workflows(tmp_path)
    path = _ci_path(root)
    original = (
        "          python scripts/verify_build_authority.py > /dev/null\n"
...
def test_ci_contract_rejects_removed_post_build_sbom_check(tmp_path: Path) -> None:
    root = _copy_workflows(tmp_path)
    path = _ci_path(root)
    marker = (
