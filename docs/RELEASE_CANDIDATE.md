...
- both wheels have the expected distribution/version filename and are byte-identical; and
- immediately before artifact publication, the live remote `refs/heads/main` still resolves to `RELEASE_SUBJECT_SHA`.

Any ambiguity, provider failure, source drift, or reproducibility failure makes the release-candidate run non-PASS.

## Evidence ownership

Release evidence is materialized under a fresh mode-0700 directory beneath the hosted runner's `RUNNER_TEMP`, not inside the repository checkout. This avoids letting a repository-owned `artifacts/` path, symlink, or pre-existing workspace object redirect release evidence persistence.
...
- `main-revalidation.json` recording the terminally observed live `main` SHA;
...
The canonical manifest explicitly records `publishing_authority: none` and `signature_claim: none`. Artifact upload is not `always()` guarded: a failed or stale candidate does not publish a release-candidate artifact whose mere existence could be mistaken for successful evidence.

## Authority separation

The workflow is `workflow_dispatch` only and top-level GitHub permissions are exactly `contents: read`. Repository CI contract verification rejects trigger broadening, write permissions, secrets, `id-token: write`, package/release tooling, mutable runner aliases, dependency caching, editable installs, failed-run artifact publication, and removal of exact-main/source/build/reproducibility/terminal-main checks.
...
4. Require terminal workflow success and the single expected release-candidate artifact. Failed runs intentionally publish no release-candidate artifact.
5. Retain the workflow run identity, exact `main` SHA, artifact identity/digest, manifest, terminal-main observation, and checksums as release-preparation evidence.
6. If `main`, version, or package bytes change, run the candidate process again. Older evidence does not certify the new bytes.
