...
| Automatic browser runtime | preinstalled hosted `/usr/bin/google-chrome`, observed before the reference test; no automatic browser/OS installer authority | browser job log records hosted Chrome version + deterministic localhost reference-SUT JUnit evidence |
...
The automatic reference-SUT browser job consumes the browser already present on the GitHub-hosted `ubuntu-24.04` image instead of installing one during untrusted PR execution. Its exact reviewed step requires `/usr/bin/google-chrome` to exist and records `google-chrome --version`; the integration test selects only Playwright's `chrome` channel through the bounded `BrowserProbe(use_system_chrome=True)` option. The runtime API does not accept an arbitrary browser executable path or arbitrary channel.

`scripts/verify_ci_contract.py` requires that exact hosted-Chrome observation step and rejects automatic workflow tokens for `playwright install`, `sudo`, `apt-get`, and `apt install`. This prevents the previous browser bootstrap from silently returning when the workflow blob constant is intentionally refreshed: automatic browser validation no longer switches to root, refreshes mutable APT repositories, or downloads Chrome/FFmpeg/headless-shell payloads from Playwright's CDN.

The hosted Chrome binary is **not** repository-pinned or cryptographically attested. Its version follows the mutable hosted runner image, so the observed executable/version is an environment-owned input. Absence or incompatibility is a browser-validation failure/incomplete condition; automatic CI must not repair it by acquiring privileged OS/browser-install authority.
...
`scripts/verify_ci_contract.py` freezes the complete automatic-workflow Git blob identity in addition to the dependency-install brackets, pre-build authority, immediate automatic-project-install guards, hosted-browser authority, runtime-SBOM digest export, isolated bare-Git archive construction, per-archive build-authority checks and matching persisted evidence, reproducible-build shell block, exact-event runtime-container build-context construction, supply-chain evidence set, and their safety-critical ordering. Replacing `$GITHUB_SHA` with mutable `HEAD`, re-enabling Git replacement objects, removing the clean Git environment, fresh Git view/object-directory/empty-template controls, reintroducing ambient archive attributes, allowing dirty tar extraction authority, removing either side of an automatic dependency-install authority bracket, restoring automatic privileged browser/OS installation, removing either archive verification or its equality/evidence requirement, removing the SBOM lineage checks, moving project installation ahead of build-authority validation, adding an alternate unreviewed project-install command, reverting the runtime container to mutable checkout `docker build ... .`, or removing required evidence causes deterministic CI-contract failure **inside a run that executes this verifier**.
...
- hosted browser/runner OS packages supplied by the GitHub-hosted image;
...
| Automatic browser validation avoids PR-time privileged browser/OS installation | exact hosted-Chrome observation step + `use_system_chrome=True` reference test + CI-contract denial of Playwright installer/sudo/APT authority | immutable/attested hosted Chrome bytes, stable browser version across runner-image updates, or browser supply-chain provenance |
...
A dependency, lock definition, Action, build-backend/configuration, project-install metadata/resource budget, browser-runtime contract, interpreter, automatic-workflow definition, Dockerfile, container build-context contract, or container-base update is a controlled engineering change—not a background resolver event. The update should:
...
4. explicitly review any proposed change to exact lock blob authority, build backend/configuration, distribution name, console/GUI scripts, project entry-point groups, file-valued project inputs, selected package-tree shape/symlink/resource policy, installed Hatch plugin surface, source-execution extension points, automatic browser-runtime/installer authority, automatic CI command surface, Docker runtime-composition instructions, or exact-event container build-context construction;
