...
An explicit `$schema` dialect identifier is authoritative input, not a hint that may be silently reinterpreted. Unsupported or malformed explicit dialect identifiers are `NOT_VERIFIED`; supported legacy dialects such as Draft-07 remain valid. Actual schema-bearing subresources are walked using the active JSON Schema specification, so an embedded resource cannot silently switch to an unknown or different dialect. `$schema`-shaped objects used only as literal instance data are not promoted into schema authority. Boolean schemas remain valid JSON Schema values.

Schema checking and instance validation execute in a fresh isolated Python worker rather than borrowing signal/timer authority from the live Agent SDK process. The parent serializes one bounded JSON payload into a private temporary workspace, launches the current Python executable with isolated startup (`-I -S`), supplies only interpreter-owned installed dependency roots for `jsonschema`/`referencing`, and uses the shared restricted subprocess environment and process-tree cleanup boundary. Worker output is bounded to 8,192 bytes, and the serialized worker input has a 2,100,000-byte defense-in-depth ceiling in addition to the live raw-field limits.
