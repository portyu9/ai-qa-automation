...
Before dependency import or payload evaluation, the worker also establishes a private **1 GiB address-space ceiling** through `RLIMIT_AS`. It never raises an inherited stricter soft or hard limit: the effective ceiling is the minimum of the project-owned 1 GiB maximum and any finite pre-existing process limits, then both child soft/hard limits are reduced to that effective value. The child re-reads the installed limits and fails closed if they are absent, unlimited, invalid, or above the project ceiling. A platform without this resource-limit authority is `BLOCKED` rather than running hostile schema evaluation without a memory envelope.

The worker owns a private **2-second `ITIMER_REAL`/`SIGALRM` evaluation budget** covering dialect inspection, schema checking, and instance validation, including hostile regular expressions and combinatorial schemas. The parent allows a separate **5-second startup/cleanup grace** around that evaluation budget so interpreter/dependency startup does not silently consume validation time; this grace cannot extend schema evaluation because the child timer is authoritative once evaluation begins. Parent interval timers, signal handlers, and caller-thread identity are not borrowed or replaced. If the child cannot provide its private evaluation timer or memory authority, execution is `BLOCKED`; evaluation timeout, memory exhaustion, or an incomplete/invalid worker result is `NOT_VERIFIED`; worker startup/cleanup failure is `BLOCKED`. Only a completely evaluated matching payload inside both execution budgets can produce `PASS`.
