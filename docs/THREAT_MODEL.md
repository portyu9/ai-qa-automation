...
This threat model assumes an AI QA agent can encounter malicious or misleading content in the target repository, rendered application, test output, API responses, and external engineering systems. The design therefore treats **prompt injection and model error as expected threat inputs**, not exceptional events.
...
| Test-intent weakening | patch-quality rules block skip/xfail/sleep/timeout/assertion weakening/suppression patterns; test-quality review |
...
| Excessive MCP privilege | tool-level read/write/destructive policy; GitHub server read-only defense in depth; unknown actions not auto-approved |
...
| Production load incident | production target denial; injected target binding; k6 script restrictions; external egress precondition |
| Unbounded/autonomous loop | independent turn/tool/network/mutation/repetition/time/cost limits + per-tool failure circuits |
| Cross-run evidence contamination | run-scoped artifact/evidence/state directories and run IDs |
| Persisted record tampering | artifact hashes; hash-chained runtime journal; optional regulated audit chain; verification surfaces corruption |
...
- a k6 script importing a remote module or pointing at a production-like host.
