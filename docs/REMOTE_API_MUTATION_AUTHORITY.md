...
The final transport request is bound to the authority that was classified. `ApiProbe.request()` does not admit post-classification `params`, request bodies, auth/cookie helper objects, extensions, or other arbitrary HTTPX request modifiers. Query parameters must be present in the URL that policy classified. Caller-supplied headers are ingested with count/byte limits before HTTPX normalization and are restricted to a small observation-oriented allowlist (`Accept`, `Accept-Encoding`, `Accept-Language`, `Authorization`, `Cache-Control`, conditional request headers, `Range`, and `User-Agent`). Unreviewed/custom headers—including `Host`, `Cookie`, body-shaping/transfer headers, action headers, and HTTP method-override headers—are rejected before transport. `Accept-Encoding` is always forced to `identity`. This prevents a caller from obtaining approval for one read-only URL/method and then changing the effective target, body, query, method, or opaque custom action semantics afterward.
...
The stronger invariant is: **generic HTTP observation cannot become autonomous remote-mutation authority through a method flag, adapter constructor, post-classification request modifier, unreviewed request header, encoded action URL, method-override header, transport ambiguity, or model output.**
