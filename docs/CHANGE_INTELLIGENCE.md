...
- every stage-zero tracked path is treated as a bounded raw-byte/mode verification subject rather than trusting Git's mutable stat cache or invoking worktree comparison commands; regular-file bytes are compared directly with the index blob identity under per-file and aggregate budgets, and exceeding those budgets makes observation explicitly incomplete rather than proving cleanliness;
