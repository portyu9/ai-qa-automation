...
Dependency promotions avoid the platform's approval-bound `pull_request` startup path by creating the generated PR against a temporary exact-main staging base outside the `main` pull-request filter, revalidating and retargeting that exact PR to `main`, and then removing the staging ref. They do not treat bot-originated pull-request runs or recursive workflow dispatch as merge evidence. The existing serialized Trusted PR Auto schedule independently selects an exact-current-main promotion, re-proves its GitHub Actions identity and signed Dependabot lineage, validates the exact prospective merge, executes the full trusted validation graph including bot CodeQL, supply chain, security gates, Python 3.11/3.14, deterministic controls, and the Playwright reference SUT, and publishes the App-bound `Trusted PR Gate`. The promotion merge controller then fresh-rebinds the live PR/base/head/merge subject and requires that exact schedule-owned App status immediately before its SHA-guarded merge. Staging metadata is liveness/recovery state only and cannot authorize validation or merge.
