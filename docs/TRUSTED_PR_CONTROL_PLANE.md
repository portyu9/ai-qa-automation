...
Bot identity alone is not authority. A serialized five-minute schedule running trusted default-branch bytes remains the bounded polling path and discovers at most one governed-bot candidate per reconciliation pass. Discovery is same-repository and exact-current-main scoped, prioritizes security auto-heal before Dependabot Actions and dependency promotions, and fresh-GETs each candidate before selection. A candidate must be open, non-draft, definitively mergeable, and match the canonical bot identity plus reviewed branch grammar before subject resolution proceeds.

Dependency promotions additionally have an event-driven qualification path because GitHub requires approval for `pull_request` runs created by the repository `GITHUB_TOKEN`. Governance creates the generated PR against a temporary exact-main staging base outside the `main` pull-request filter, revalidates and retargets that exact PR to `main`, removes the staging ref, then dispatches the reviewed default-branch CI and CodeQL workflows with the exact generated head/ref as explicit inputs. Those workflows execute trusted `main` workflow bytes, validate the requested promotion namespace and exact subject, and publish exact-head `Required PR Gate` and `CodeQL` checks whose external IDs bind the originating workflow run and attempt. A successful qualification check may wake Trusted PR Auto through `workflow_run`; admission becomes eligible only when both independently re-fetched exact qualifications are green. The promotion merge controller then re-proves those checks, the live PR/base/head/merge subject, and the dedicated-App `Trusted PR Gate` immediately before its SHA-guarded merge. Ordinary Dependabot action merges retain the stricter schedule-only gate requirement. Mutable staging/retry metadata is liveness state only and cannot authorize validation or merge. Qualification intent is persisted and exact-subject-revalidated before dispatch; an observed terminal validation failure is non-retryable, and multiple exact qualification checks are ambiguous evidence that fails closed.
