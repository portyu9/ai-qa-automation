...
Bot identity alone is not authority. A serialized five-minute schedule running trusted default-branch bytes remains the bounded polling fallback and discovers at most one governed-bot candidate per reconciliation pass. Discovery is same-repository and exact-current-main scoped, prioritizes protected/security remediation before Dependabot Actions and dependency promotions, and fresh-GETs each candidate before selection. A candidate must be open, non-draft, definitively mergeable, and match the canonical bot identity plus reviewed branch grammar before subject resolution proceeds.

Dependency promotions avoid the platform's approval-bound `pull_request` startup path by creating the generated PR against a temporary exact-main staging base outside the `main` pull-request filter, revalidating and retargeting that exact PR to `main`, and then removing the staging ref. They do not treat bot-originated pull-request runs, recursive workflow dispatch, or the wake check itself as merge evidence. When an exact promotion lacks terminal App-owned gate evidence, trusted default-branch dependency governance may publish one neutral `Dependency Promotion Qualification Wake` check bound to the promotion head, current-main base, governance run ID, and run attempt. Only after that exact governance run completes successfully may the default-branch Trusted PR Auto workflow consume its `workflow_run` event; failed runs, non-neutral checks, wrong App provenance, stale subjects, and ambiguous matches remain non-admissible. The five-minute schedule remains an independent fallback.

Whether awakened by that reviewed governance event or by the schedule, Trusted PR Auto re-proves the governed-bot identity and signed Dependabot lineage, validates the exact prospective merge, executes the full trusted validation graph including bot CodeQL, supply chain, security gates, Python 3.11/3.14, deterministic controls, and the Playwright reference SUT, and only then publishes the dedicated-App `Trusted PR Gate`. The promotion merge controller fresh-rebinds the live PR/base/head/merge subject and accepts only exact current-main Trusted PR Auto evidence from the reviewed `schedule` or `workflow_run` event classes before its SHA-guarded merge. The neutral wake and staging metadata are liveness/recovery state only and cannot authorize validation or merge.
