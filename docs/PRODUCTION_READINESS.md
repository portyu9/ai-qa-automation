...
Automatic protected-merge authorization is a default-branch trust path in `.github/workflows/trusted-pr-auto.yml`, woken by reviewed CI or CodeQL completion through `workflow_run`. The wake-up payload is not authority. Owner-routine PRs require exact identity and zero protected-root drift. Three finite governed-bot lanes—Dependabot GitHub Actions updates, deterministic dependency promotions, and CodeQL auto-heal repairs—may cross reviewed roots only after canonical bot identity, branch/source lineage, trusted-main CI + CodeQL qualification, lane-specific policy proof, exact current base, and prospective-merge validation. The full deterministic validation graph executes against the prospective merge, admission and bot authority are re-proved immediately before publication, and only the terminal reporter enters Environment `trusted-pr-gate` to obtain the dedicated GitHub App credential and publish an exact PR/base/head/merge-bound `Trusted PR Gate`.
