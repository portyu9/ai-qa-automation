...
8. generated repair commits immutably bind the route-record digest, route-plan digest, and GitHub-reported route-artifact digest in validated commit-message trailers, while generated repair markers persist the complete canonical route record plus those same plan/artifact digests and originating run/artifact identity; duplicated marker subject/decision/authority/strategy fields must exactly match that self-digesting record;
9. later repair admission recomputes the same live route, rejects missing route provenance, requires all three immutable repair-commit trailers to equal the revalidated route-record plus marker plan/artifact digests, and requires successful originating controller-run/artifact provenance before existing diff, CodeQL, trusted-gate, and merge guards apply;
10. first publication of durable terminal closure re-verifies the marker's canonical route record against its own digest, requires that digest plus plan/artifact digests to equal the immutable repair-commit trailers, and requires the still-available successful originating route artifact/run; schema-v2 terminal evidence then embeds that same canonical route record and exact route-plan/run/artifact provenance alongside post-merge CI, CodeQL, trusted-gate, merge, and fixed-alert evidence. Later replay accepts artifact expiry only after the existing unedited GitHub-Actions certificate still reproduces the canonical route record and statically rebinds to that exact merged subject while durable CI/CodeQL/gate/controller evidence continues to validate.
