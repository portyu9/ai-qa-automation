...
8. generated repair commits immutably bind the route-record digest, route-plan digest, and GitHub-reported route-artifact digest in validated commit-message trailers, while generated repair markers bind those same digests plus routing-policy version, originating workflow run/attempt, artifact identity, and the Autofix evidence state used by the route;
...
10. first publication of durable terminal closure re-verifies those immutable commit digests and the still-available successful originating route artifact/run, then emits schema-v2 terminal evidence carrying the same route-record digest, route-plan digest, route run/attempt, artifact identity/digest, decision, authority, policy version, and Autofix evidence alongside exact post-merge CI, CodeQL, trusted-gate, merge, and fixed-alert evidence; later replay accepts artifact expiry only after the existing unedited GitHub-Actions certificate statically rebinds to that exact merged subject and continues to revalidate the durable CI/CodeQL/gate/controller evidence.
