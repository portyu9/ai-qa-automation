...
That distinction prevents historical, partial, artifact-only, build-config-expanded, plugin-expanded, source-symlink-expanded, SBOM-substituted, replacement-object-retargeted, checkout-Git-metadata-retargeted, workflow-command-surface-expanded, or wrong-subject evidence from becoming merge/release authority it does not possess.
